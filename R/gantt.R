...
#' means right-justified. (Left-justification works poorly in RStudio, but
...
setMethod(
    ...
    definition = function(x, xlim,
        ...
        if (debug) {
            cat("range: ", as.character(r[1]), "to", as.character(r[2]), "\n")
        }
        ...
        for (i in 1:ndescriptions) {
            if (ylabels$justification[i] == 1) {
                left <- par("usr")[1]
                text(left, topdown[i], x[["description"]][i],
                    pos = 2,
        ...
        for (i in 1:ndescriptions) {
            if (!is.na(x[["start"]][i])) {
                ...
                if (debug) {
                    cat(as.character(x[["description"]][i]), "takes", as.numeric(difftime(as.POSIXct(x[["end"]][i]), as.POSIXct(x[["start"]][i]), units = "secs")), "s\n")
                }
                ...
                if (debug) {
                    cat(as.character(x[["description"]][i]))
                    cat(" done=", x[["done"]][i], " mid=")
                    print(mid)
                    cat(" left=")
                    print(left)
                    cat("right=")
    ...
    definition = function(object, ...) {
        if (length(object@data[[1]])) {
            ...
            for (t in 1:num.descriptions) {
                ...
                cat(
                    paste(format(object[["key"]][t], width = 3, justify = "right"), ",", sep = ""),
                    paste(as.character(object[["description"]][t]), ",",
                        spacer,
                        format(object[["start"]][t]), ", ",
                        object[["end"]][t], ", ",
                        format(object[["done"]][t], width = 4, justify = "right"),
                        sep = ""
                    )
                )
...
#'     ),
#'     c(
#'         startT1, startT1, startT2, startQE, startProposal, startProposal,
#'         startThesisWork, startWriting
#'     ),
#'     c(
#'         startT1, endT1, endT2, endQE, startProposal, endProposal,
...
read.gantt <- function(file, debug = FALSE) {
    ...
    while (TRUE) {
        tokens <- trimws(scan(file,
            what = character(0), nlines = 1,
