...
\code{\link{gantt-class}}. If not, the best is to try some other software,
because only minimal customization is permitted by the
...
There are three ways to create objects of \code{\link{gantt-class}}: (a) write
information in a text file and use \code{\link{read.gantt}}, (b) use
\code{\link{as.gantt}} to assemble the object in one (somewhat complicated) step, or
(c) use \code{new} to create an object and then \code{\link{ganttAddTask}} to
add tasks one by one. The third approach is probably the easiest for beginners,
so it is illustrated below, with a sketch of courses typically taken in
postgraduate oceanography programs.
